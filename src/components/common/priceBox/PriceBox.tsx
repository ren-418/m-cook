...
export default function PriceBox({ inline = false, ...props }: PriceBoxProps) {
  return (
    <div className={inlineStyles({ inline: inline })}>
      {props.discount > 0 && <div className="flex justify-end items-center text-small gap-1 align-middle">
      ...
      <span className="text-right text-neutral-800 text-body-b align-middle">
        {currencyMap[props.currency]}{props.actualPrice - (props.actualPrice * props.discount / 100)}
      </span>
    </div>
  );
}
